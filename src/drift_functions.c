/*-------------------------------------------------------------------------
-                     d r i f t _ f u n c t i o n s . c                   -
---------------------------------------------------------------------------
  ...
  Segments linear fallend auf ein Zufallsniveau aus U(-1, -1/4)
  (Incremental Drift), an der Segmentgrenze Sprung auf ein Zufallsniveau
  aus U(1/4, 1) (Sudden Drift). Das erste Segment startet bei 0.

  Rueckgabewert normiert auf [-1, 1]; die Amplitude skaliert der Aufrufer
  (IDV(29)-Wert). Zustandslos: der Wert haengt nur von t und den Parametern
  ab, Zufallsniveaus werden deterministisch aus seed und Segmentindex
  abgeleitet (splitmix64). Damit liefern Notebook (ctypes) und S-Function
  (per #include) unabhaengig von Solver-Schrittweite identische Verlaeufe.
...
DRIFT_API double drift_signal(double t, double period, double gamma,
  ...
  /* Startniveau (Segment 0: 0) und Zielniveau des Segments */
  if (k == 0){
    c0 = 0.0;
  }else{
    c0 = drift_uniform(seed, 2000u + (unsigned long long)k, 0.25, 1.0);
  }
  c1 = drift_uniform(seed, 3000u + (unsigned long long)k, -1.0, -0.25);

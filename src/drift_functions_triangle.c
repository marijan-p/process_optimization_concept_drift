/*-------------------------------------------------------------------------
-            d r i f t _ f u n c t i o n s _ t r i a n g l e . c          -
---------------------------------------------------------------------------
  Dreieck-Driftsignal als Alternative zum Saegezahn (drift_functions.c),
  ...
  in temexd_mod_drift.c:
    #include "drift_functions.c"           -> Saegezahn
    #include "drift_functions_triangle.c"  -> Dreieck
...
DRIFT_API double drift_signal(double t, double period, double gamma,
  ...
  if (tau < period){
    return 1.0 - 2.0 * tau / period;
  }
  return -1.0 + 2.0 * (tau - period) / period;
}

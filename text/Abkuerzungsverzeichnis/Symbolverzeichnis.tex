...
\begin{longtable}{@{}p{0.20\textwidth}p{0.74\textwidth}@{}}
    ...
    $\lambda$                                  & Schwellenwert \\                         % $\lambda_{\mathrm{bin}}$ Binarisierung, $\lambda_{\mathrm{ret}}$ Nachtraining, $\lambda_{\mathrm{rec}}$ Recurrence-Selektor, $\lambda$ KSWIN
...
\begin{longtable}{@{}p{0.20\textwidth}p{0.74\textwidth}@{}}
    ...
    %: $n_x, n_y, n_u$ (Signaldimensionen), $n_\theta$ (Parameterdimension), $n_a, n_b$ (Modellordnung), $n, n_0, n_1$ (ADWIN-Fenster), $n_{\mathrm{w}}$ (Adaptionsfenster), $n_{\mathrm{p}}$ (Adaptionsperiode), $n_{\mathrm{h}}$ (Rückblickhorizont des Recurrence-Selektors), $n_{\mathrm{tol}}$ (Toleranzfenster), $n_{-}, n_{+}$ (Nachtrainingsfenster)
    \\
    $N_{\ast}$ & Anzahl
    %: $N$ (Stichproben), $N_{\mathrm{in}}$ (Eingangskanäle), $N_{\mathrm{q}}$ (Rauschquellen), $N_c$ (Änderungsindizes), $N_{\mathrm{seg}}$ (Segmente), $N_{\mathrm{sd}}$ (Sudden Drifts), $N_{\mathrm{fa}}$ (Fehlalarme), $N_{\mathrm{train}}$ (Nachtrainings), $N_{\mathrm{chunk}}$ (Chunks), $N_{\mathrm{rec}}$ (zurückgeholte Punkte) 
    \\

...
%
Damit folgen die Betriebskosten weiterhin dem unveränderten Prozess, während der aufgezeichnete Messwert zunehmend von ihm abweicht. Ein und derselbe beobachtete Wert der Strippertemperatur gehört im Verlauf des Drifts zu immer anderen Betriebskosten, sodass sich die bedingte Verteilung $P(y \mid X)$ verschiebt und nach der Systematik gemäß Tabelle~\ref{tab:cd_definitionen} \emph{Real Concept Drift} gemäß \eqref{eq:real_concept_drift} vorliegt. Der aufgezeichnete Messwert wird dabei mit dem Faktor $1 + \kappa_m \cdot c(t)$ skaliert, worin $\kappa_m$ den Driftkoeffizienten und $c(t)$ das im Folgenden beschriebene Driftsignal bezeichnet. Da dieser Faktor multiplikativ wirkt, hängt der absolute Fehler vom Betriebspunkt ab. \par
%
Das Driftsignal ist dabei strukturgleich zum synthetischen Signal $c[k]$ aus Abschnitt~\ref{sec:syn_drift}, wird jedoch als $c(t)$ zeitkontinuierlich formuliert. Da das Simulink-Modell den Solver \texttt{ode45} mit veränderlicher Schrittweite verwendet, liegen die Aufrufzeitpunkte auf keinem festen Raster.
Innerhalb eines Segments fällt es gemäß \eqref{eq:cd_incremental} jedoch weiterhin linear auf ein Zielniveau des unteren Bandes ab, an der Segmentgrenze springt es auf das Startniveau des folgenden Segments, sodass beide Drifttypen aus Abbildung~\ref{fig:phenomenon} auch im \ac{TEP} abgebildet sind.
Die Segmentgrenzen folgen einem festen Raster mit der nominellen Segmentdauer $T_{\mathrm{seg}}$,
%
\begin{equation}
    \tau_i = i \cdot T_{\mathrm{seg}} + \gamma_{\tau} \cdot \frac{T_{\mathrm{seg}}}{2} \cdot \bigl(2 \, u_i - 1\bigr),
    \qquad u_i \sim \mathcal{U}(0,\, 1)
    \label{eq:tep_drift_boundaries}
\end{equation}
%
wobei $\gamma_{\tau}$ denselben Verschiebungsanteil bezeichnet wie im synthetischen Fall. Alle Zufallsniveaus werden deterministisch aus einem Seed und dem Segmentindex abgeleitet, womit das Signal zustandslos und unabhängig von der Schrittweite des Solvers reproduzierbar ist. Es steht damit zugleich als Referenz für die Bewertung der Detektion in Abschnitt~\ref{sec:tep_detektion} zur Verfügung. \par
%
Die Segmentdauer ist mit $T_{\mathrm{seg}} = \SI{1752}{\hour}$ so gewählt, dass fünf Segmente je Kalenderjahr resultieren, was der Segmentzahl im synthetischen Datenstrom entspricht und zugleich als Kalibrierintervall interpretiert werden kann. Nach unten ist sie dadurch begrenzt, dass sich der Kalibrierfehler während des Anfahrens und Haltens eines Betriebspunkts nur unwesentlich ändern darf, damit jeder Betriebspunkt einen einzelnen Driftpegel abbildet. Der Driftkoeffizient $\kappa_m$ ist so bemessen, dass die Verschiebung des Prädiktionsfehlers deutlich über dem Rauschen des Modells innerhalb eines Betriebspunkts liegt, der Kalibrierfehler mit einem Spitzenwert von zehn Prozent aber klein genug bleibt, um nicht bereits durch einen Plausibilitätsabgleich mit den übrigen Messgrößen aufzufallen. \par
%

...
%
Formal erstreckt sich das Signal über eine Gesamtlänge von $K$ Zeitschritten. Die Wahl der Abtastfrequenz orientiert sich dabei am Ausgangsbeispiel der Mühle mit GPlink, das mit einer Frequenz von $\paramFs$ arbeitet. Die entsprechende Abtastzeit $T_s = \paramTs$ wird auf ein Kalenderjahr angewandt. Die Segmentierung dieses Zeitbereichs folgt einem festen Raster mit der nominellen Segmentdauer $T_{\mathrm{seg}} = \paramTseg$, die als Wartungs- beziehungsweise Kalibrierintervall zu verstehen ist. Im betrachteten Kalenderjahr liegen damit $\paramNSeg$ Driftsegmente vor. Da ein Wartungstermin in der Praxis um einige Tage schwankt, werden die inneren Segmentgrenzen $\tau_1 < \tau_2 < \cdots$ gegenüber dem Raster um maximal $\pm \gamma_{\tau} \cdot T_{\mathrm{seg}}/2$ zufällig verschoben, wobei $\gamma_{\tau}$ den Verschiebungsanteil bezeichnet. Zusammen mit dem festen Anfangspunkt $\tau_0 = 0$ und dem Ende des Zeitbereichs bei $K$ sind damit alle Segmentgrenzen definiert. \par
%
...
\end{itemize}
%
Der \ac{TEP} verfügt über 41 Messgrößen und 12 Stellgrößen, wobei der Großteil der Stellgrößen der Ventilsteuerung dient. Die Messgrößen werden dabei als \texttt{XMEAS(1)} bis \texttt{XMEAS(41)} bezeichnet. \newline
...
%
Die Segmentdauer $T_{\mathrm{seg}}$ wird aus dem synthetischen Fall übernommen. Nach unten ist sie dadurch begrenzt, dass sich der Kalibrierfehler während des Anfahrens und Haltens eines Betriebspunkts nur unwesentlich ändern darf, damit jeder Betriebspunkt einen einzelnen Driftpegel abbildet. Der Driftkoeffizient $\kappa_m$ ist so bemessen, dass die Verschiebung des Prädiktionsfehlers deutlich über dem Rauschen des Modells innerhalb eines Betriebspunkts liegt, der Kalibrierfehler mit einem Spitzenwert von zehn Prozent aber klein genug bleibt, um nicht bereits durch einen Plausibilitätsabgleich mit den übrigen Messgrößen aufzufallen. \par
%

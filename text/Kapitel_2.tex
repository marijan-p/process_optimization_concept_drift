...
\begin{figure}[htb!]
    \vspace{5mm}
    \centering
    \includesvg[width=\linewidth]{\graphdir Mühlen_in_Zementindustrie.svg}
...
\end{figure}
%
Im Laufe der Zeit haben sich zur Vermahlung mehrere Verfahren etabliert. Neben der Kugelmühle wird hauptsächlich die \ac{VRM} eingesetzt, die aufgrund ihres hohen Wirkungsgrads als Stand der Technik gelten \cite{schaefer.2001}. Dennoch gibt es auch bei \ac{VRM} ein erhebliches Potenzial zur Energieoptimierung. So kann beispielsweise eine modulare Bauweise der Mühle den Prozess durch eine Reduzierung der Stillstandzeiten optimieren \cite{woywadt.2017}. Neben der Zementindustrie kommen \ac{VRM} auch in der Kohleindustrie zum Einsatz. Zu den gängigen Mahlgütern zählen Kalkstein, Kohle, Ton, Gips und Schlacke.
...
Der prinzipielle Aufbau einer \ac{VRM} ist in Abbildung \ref{fig:muehle} schematisch dargestellt. Grundsätzlich besteht eine \ac{VRM} aus einem rotierenden Mahlteller, auf dem die Walzen durch ihr Eigengewicht nach unten gedrückt werden. Häufig wird die Kraft in Richtung Mahlteller durch ein hydraulisches System unterstützt. Die Verschleißteile, einschließlich der Walzen und der Oberfläche des Mahltellers, bestehen in der Regel aus austauschbaren Teilen aus Hartgusslegierungen wie NiHard, die für ihre extreme Härte bekannt sind.
%
\begin{figure}[hbt!]
    \centering
    \hspace*{-4cm}
    \includesvg[width=0.8\linewidth]{\graphdir MVR-C_6.svg}
    \caption{Schematische Abbildung einer \ac{VRM} vom Typ MVR der Firma Gebr. Pfeiffer SE}
    \label{fig:muehle}
\end{figure}
%
...
\begin{figure}[hbt!]
    \centering
    \includesvg[width=0.9\linewidth]{\graphdir GPlink.svg}
...
Die prinzipielle Funktionsweise des Systems ist in Abbildung \ref{fig:gplink} schematisch dargestellt. Die Sensordaten der Mahlanlage werden zunächst konventionell über \ac{SPS} des Kunden erfasst. Ergänzend hierzu ist eine zusätzliche \ac{SPS} installiert, die so genannte GPlink-\ac{SPS}. Diese übernimmt zwei zentrale Aufgaben: Zum einen empfängt sie ausgewählte Prozesssignale aus der Kunden-\ac{SPS} mit einer Frequenz von $\SI{1}{\hertz}$, zum anderen stellt sie zusätzliche Messdaten aus einer erweiterten Sensorik bereit, die Bestandteil von GPlink ist, und überträgt diese an die Kunden-\ac{SPS} zurück. Auf diese Weise wird die bestehende Automatisierungsstruktur erweitert, ohne sie grundlegend zu verändern.
Die von der GPlink-\ac{SPS} erfassten Daten werden über eine OPC-UA-Schnittstelle an einen \ac{IPC} übertragen. Der \ac{IPC} wiederum fungiert als Bindeglied zwischen der lokalen Automatisierungsebene und der Cloud-Infrastruktur. Die Daten werden so an cloudbasierte Dienste weitergeleitet, wo sie gespeichert und weiterverarbeitet werden.
In der Cloud sind mehrere, mittels Docker containerisierte Dienste implementiert. Dazu zählen unter anderem \emph{Kafka} zur Datenübertragung, \emph{InfluxDB} zur Speicherung der Zeitreihendaten, \emph{PostgreSQL} zur Ablage verarbeiteter Daten, \emph{Grafana} zur Visualisierung sowie \emph{Python}-basierte Dienste zur Datenanalyse. Die vom \ac{IPC} gesendeten Prozessdaten werden über Kafka in die Cloud übertragen und in einer Influx-Datenbank gespeichert. Auf dieser Datenbasis werden mit Python Berechnungen, Analysen und Auswertungen durchgeführt, deren Ergebnisse anschließend in einer PostgreSQL-Datenbank gespeichert werden. 
...
Für die Operatoren im \ac{CCR} stehen die Prozessdaten somit auf zwei Wegen zur Verfügung: Zum einen weiterhin direkt über die Visualisierung der Kunden-\ac{SPS}, zum anderen ergänzend über GPpro mit erweiterten Analyse-, Historisierungs- und Visualisierungsfunktionen. GPlink/GPpro erweitert damit die klassische Automatisierungsebene um eine digitale Service- und Analyseplattform und bildet eine Grundlage für datengetriebene Prozessüberwachung und -optimierung.
%
...
Durch die prominente Präsenz großer Sprachmodelle (engl. \ac{LLM}) im öffentlichen Diskurs finden die Begriffe „Künstliche Intelligenz“, „Machine Learning“ und „neuronale Netze“ zunehmend Verwendung im allgemeinen Sprachgebrauch. Diese Begriffe bezeichnen jedoch eine Vielzahl unterschiedlicher Modellklassen, deren inhaltliche Breite durch eine Reduktion auf \ac{LLM}s nicht angemessen abgebildet wird. Gleichzeitig ist im industriellen Kontext zu beobachten, dass zum Teil auch vergleichsweise einfache oder etablierte algorithmische Ansätze unter diesen Begriffen subsumiert werden, was zu einer begrifflichen Verwässerung beiträgt und die fachliche Trennschärfe weiter reduziert. 
...
Dabei sei als erstes die \ac{LR} aufgeführt, die den Zusammenhang zwischen einer abhängigen Zielgröße und einer oder mehreren unabhängigen Inputvariablen durch eine lineare Gleichung beschreibt. Sie ist leicht interpretierbar, bei komplexeren, nicht-linearen Zusammenhängen stößt sie jedoch an ihre Grenzen und kann hier lediglich zur Approximation dienen. \newline
Die Lasso-Regression erweitert dieses Konzept der \ac{LR} um einen Regularisierungsterm \cite{tibshirani.1996}. Dieser bestraft große Modellgewichte und kann diese auf Null setzen, was nicht nur Overfitting entgegenwirkt, sondern auch zu einer automatischen Feature-Auswahl führt. \par
...
Die grundlegendste Form eines künstlichen neuronalen Netzes ist das \ac{MLP}, welches zur Klasse der Feedforward Neural Networks gehört \cite{goodfellow.2016}. Der Informationsfluss in einem \ac{MLP} ist streng gerichtet und verläuft von der Eingabe zur Ausgabe, ohne Rückkopplungen.
...
Klassischerweise ist das \ac{MLP} ein statisches Modell. Um es für die Regression von Zeitreihendaten nutzbar zu machen und Dynamiken zu erfassen, kann ein Sliding-Window-Ansatz verwendet werden \cite{bontempi.2012}. Dabei wird ein Datenfenster festgelegter Breite aus vergangenen Zeitschritten zu einem Vektor transformiert und dem \ac{MLP} als statische Eingabe übergeben. So kann das Modell historische Abhängigkeiten in seine Vorhersage einbeziehen. \par
Dem \ac{MLP} gegenüber stehen neuronale Architekturen, die zeitliche Abhängigkeiten nicht ausschließlich über die Eingangsrepräsentation, sondern auch über ihre interne Struktur erfassen.
\ac{CNN} wurden ursprünglich für die Bilderkennung entwickelt \cite{lecun.2002}. Sie nutzen Faltungsschichten (engl. Convolutional Layers), in denen Filter über die Eingabedaten gleiten, um lokale Features zu extrahieren, wobei die Gewichte geteilt werden.
Obwohl ursprünglich für 2-dimensionale Bilddaten konzipiert, lassen sich \ac{CNN}s auf Zeitreihendaten anwenden. Dabei werden 1D-\ac{CNN}s eingesetzt, bei denen ein Filter entlang der Zeitachse gleitet \cite{kiranyaz.2021}. Dadurch erkennt und extrahiert das Modell automatisch lokale zeitliche Muster und Trends innerhalb der Zeitreihe. \par
...
\ac{MLP} & statisch & Tanh \\
\ac{MLP} & dynamisch & Tanh \\
\ac{CNN} & dynamisch & ReLU \\
...
%
\input{\resultsdir model_comparison.tex}
%
\begin{table}[tbp]
    \centering
    ...
    \textbf{Modelltyp} & \textbf{Bester \ac{MAE}} & \textbf{$\Delta$ Best} & \textbf{Bester \ac{MAE}} &  \textbf{$\Delta$ Best} \\
        \midrule
        Lineare Regression & \mcLinRegMaeStat & \mcLinRegDeltaStat & \mcLinRegMaeTrans & \mcLinRegDeltaTrans \\
        Lasso-Regression & \mcLassoMaeStat & \mcLassoDeltaStat & \mcLassoMaeTrans & \mcLassoDeltaTrans \\
        \ac{MLP} (statisch) & \mcMlpStaticMaeStat & \mcMlpStaticDeltaStat & \mcMlpStaticMaeTrans & \mcMlpStaticDeltaTrans \\
        \ac{MLP} (dynamisch) & \mcMlpDynamicMaeStat & \mcMlpDynamicDeltaStat & \mcMlpDynamicMaeTrans & \mcMlpDynamicDeltaTrans \\
        \ac{CNN} & \mcCnnMaeStat & \mcCnnDeltaStat & \mcCnnMaeTrans & \mcCnnDeltaTrans \\
        \ac{LSTM} & \mcLstmMaeStat & \mcLstmDeltaStat & \mcLstmMaeTrans & \mcLstmDeltaTrans \\
        \bottomrule
    \end{tabular}
\end{table}
%
\begin{figure}[tbp]
    ...
    \begin{subfigure}[t]{\textwidth}
        \centering
        \input{\plotdir model_comparison_stationary.pgf}
    ...
    \begin{subfigure}[b]{\textwidth}
        \centering
        \input{\plotdir model_comparison_transient.pgf}
        \caption{Transiente Testdaten}
        \label{fig:violin_transient}
    \end{subfigure}
    \caption{Kombinierte Violin- und Boxplots der \ac{MAE}-Verteilung aller Modelle auf quasi-stationären und transienten Testdaten}
    \label{fig:violin}
\end{figure}
%
...
\begin{figure}[ht!]
    ...
    \begin{subfigure}[b]{0.345\textwidth}
        \centering
        \input{../plots/phenomenon_outlier.pgf}
        \caption{Ausreißer}
    ...
    \begin{tabular}{rc}
        ...
        Actual Drift & \parbox{10cm}{
            \begin{equation} \label{eq:actual_drift}
                P_t(y \ | \ X) \neq P_{t+\Delta t}(y \ | \ X) \text{ und } P_t(X) = P_{t+\Delta t}(X) 
            \end{equation}} \\
...
\begin{figure}[hbt!]
    \centering
    \includesvg[width=\linewidth]{\graphdir drift_detection.svg}
...
\end{equation}
%
geschätzt. Unter der Annahme eines stationären Zustands und für eine hinreichend große Anzahl an Beobachtungen kann die zugrunde liegende Binomialverteilung durch eine Normalverteilung approximiert werden. \ac{DDM} speichert dabei die bislang minimal beobachteten Fehlerraten $p_{\mathrm{min}}$ und Standardabweichungen $\sigma_{\mathrm{min}}$, die fortlaufend aktualisiert werden, sofern $p_i+\sigma_i < p_{\mathrm{min}} + \sigma_{\mathrm{min}}$.
...
Eine Erweiterung der \ac{DDM} stellt die \textbf{\ac{EDDM}} dar \cite{baena.2006}. \ac{EDDM} wurde insbesondere zur verbesserten Erkennung gradueller Konzeptdrifts entwickelt, ohne dabei bei abrupten Driftformen an Performance einzubüßen. Im Gegensatz zu \ac{DDM} analysiert \ac{EDDM} die zeitlichen Abstände zwischen aufeinanderfolgenden Fehlklassifikationen. Die zentrale Annahme ist dabei, dass ein gradueller Drift zu einer sukzessiven Verkürzung der Abstände zwischen aufeinanderfolgenden Fehlklassifikationen führt.
Für jedes Zeitintervall $i$ wird daher die mittlere Distanz zwischen zwei Fehlklassifikationen $p'_i$ sowie deren Standardabweichung $\sigma'_i$ geschätzt. Während des Lernprozesses speichert \ac{EDDM} die Referenzwerte $p'_{\max}$ und $\sigma'_{\max}$, sofern $p'_i + 2 \ \sigma'_i > p'_{\max} + 2\,\sigma'_{\max}$. Diese Referenzwerte entsprechen dem Zustand, in dem das Modell das zugrunde liegende Konzept am besten approximiert.
...
\begin{figure}[hbt!]
    \centering
    \includesvg[width=\linewidth]{\graphdir KSWIN_memory.svg}
    \caption{Speicherstrategie von \ac{KSWIN} angelehnt an \cite{raab.2020}}
    \label{fig:KSWIN_memory}
\end{figure}
%
Ein weiterer Fenster-basierter Ansatz zur Erkennung von \ac{CD} ist das \textbf{\ac{KSWIN}} von Raab et al. (2020) \cite{raab.2020}. \ac{KSWIN} basiert auf einem Gleitfenster fester Größe, das kontinuierlich die zuletzt beobachteten Datenpunkte eines Datenstroms speichert und mithilfe eines nicht-parametrischen statistischen Tests auf Verteilungsänderungen untersucht. Hierzu wird ein Fenster $W$ der Größe $n$ geführt, das mit jedem neu eintreffenden Datenpunkt aktualisiert wird, indem der älteste Wert entfernt und der neue Wert hinzugefügt wird. Aus diesem Fenster werden zwei Teilfenster konstruiert: Ein aktuelles Detektionsfenster $W_1$, das die $r$ zuletzt eingetroffenen Datenpunkte enthält, sowie ein Referenzfenster $W_0$, das ebenfalls aus $r$ Datenpunkten besteht. Diese werden uniform zufällig aus dem älteren Bereich des Fensters $W$ gezogen, der nicht zu $W_1$ gehört. Auf diese Weise soll $W_0$ die historische Datenverteilung approximieren, ohne Annahmen über deren Form zu treffen. Die Speicherstrategie ist in Abbildung \ref{fig:KSWIN_memory} veranschaulicht. \par
